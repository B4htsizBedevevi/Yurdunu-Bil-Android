...
object QuestionTaxonomy {
    ...
    fun tags(q: Question): Set<String> {
        ...
        when (q.topic) {
            ...
            "Tarım", "Hayvancılık", "Ekonomik Coğrafya" -> { result += "Tarım"; if (listOf("buğday", "arpa", "mısır", "pirinç", "tahıl").any(h::contains)) result += "Tahıllar"; if (listOf("pamuk", "tütün", "şeker pancarı", "ayçiçeği").any(h::contains)) result += "Endüstri Bitkileri"; if (listOf("zeytin", "çay", "fındık", "turunçgil", "üzüm", "incir").any(h::contains)) result += "Bahçe Bitkileri"; if (listOf("hayvancılık", "mera", "büyükbaş", "küçükbaş").any(h::contains)) result += "Hayvancılık" }
...
object SharedGameModes {
    ...
    val agriculture = SharedGameMode("agriculture", "Tarım Avı", "Ürünleri iklim ve bölgeyle eşleştir", "agriculture", 10, 150, 120, setOf("Tarım", "Ekonomik Coğrafya"))
