...
private fun AuthExperienceScreen(
    ...
    Box(
        Modifier.fillMaxSize().background(Brush.verticalGradient(listOf(AuthBg, Color(0xFF0A3029), AuthBg)))
    ) {
        LazyColumn(
            ...
            item {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    if (forgot || recovery) IconButton(onClick = { if (recovery) onBack() else { forgot = false; clearMessages() } }) { Icon(Icons.Default.ArrowBack, "Geri", tint = AuthText) }
            ...
            if (recovery) {
                item {
                    AuthCard {
                        ...
                        AuthField("Yeni şifre", newPassword, { newPassword = it; clearMessages() }, showPassword, { showPassword = !showPassword })
                        Spacer(Modifier.height(10.dp))
                        AuthField("Yeni şifre tekrar", confirmPassword, { confirmPassword = it; clearMessages() }, showPassword, { showPassword = !showPassword })
                ...
                item {
                    AuthCard {
                        ...
                        Spacer(Modifier.height(14.dp))
                        AuthButton("Şifre Yenileme Bağlantısı Gönder", busy) {
                            scope.launch {
                                busy = true; clearMessages()
                                runCatching {
                                    require(email.contains("@")) { "Geçerli bir e-posta adresi gir." }
                                    SupabaseClientProvider.client.auth.sendRecoveryEmail(email.trim(), redirectUrl = "yurdunubil://auth")
                                }.onSuccess {
                ...
                item {
                    AuthCard {
                        ...
                        EmailField(email, { email = it; clearMessages() })
                        Spacer(Modifier.height(10.dp))
                        AuthField("Şifre", password, { password = it; clearMessages() }, showPassword) { showPassword = !showPassword }
                        if (!register) {
                        ...
                        AuthButton(if (register) "Hesap Oluştur" else "Giriş Yap", busy) {
                            scope.launch {
                                busy = true; clearMessages()
                                runCatching {
                                    require(email.contains("@")) { "Geçerli bir e-posta adresi gir." }
                                    ...
                                    if (register && SupabaseClientProvider.client.auth.currentSessionOrNull() == null) message = "Hesabın oluşturuldu. E-posta kutundaki doğrulama bağlantısını kontrol et."
                                    else onDone()
                                }.onFailure { error = friendlyAuthError(it) }
                                busy = false
                            }
...
private fun AuthCard(content: @Composable ColumnScope.() -> Unit) {
    androidx.compose.material3.Card(shape = RoundedCornerShape(28.dp), colors = androidx.compose.material3.CardDefaults.cardColors(containerColor = AuthCard)) {
...
private fun EmailField(value: String, onValueChange: (String) -> Unit) {
    OutlinedTextField(value, onValueChange, Modifier.fillMaxWidth(), label = { Text("E-posta") }, placeholder = { Text("ornek@mail.com") }, leadingIcon = { Icon(Icons.Default.Email, null) }, singleLine = true)
}

@Composable
private fun AuthField(label: String, value: String, onValueChange: (String) -> Unit, visible: Boolean, toggle: () -> Unit) {
    OutlinedTextField(value, onValueChange, Modifier.fillMaxWidth(), label = { Text(label) }, leadingIcon = { Icon(Icons.Default.Lock, null) }, trailingIcon = { IconButton(onClick = toggle) { Icon(if (visible) Icons.Default.VisibilityOff else Icons.Default.Visibility, null) } }, singleLine = true, visualTransformation = if (visible) VisualTransformation.None else PasswordVisualTransformation())
}

@Composable
private fun AuthButton(text: String, busy: Boolean, onClick: () -> Unit) {
    Button(onClick = onClick, enabled = !busy, Modifier.fillMaxWidth().height(54.dp), shape = RoundedCornerShape(16.dp), colors = ButtonDefaults.buttonColors(containerColor = AuthGreen, contentColor = Color(0xFF06251B))) {
        if (busy) CircularProgressIndicator(Modifier.size(21.dp), color = Color(0xFF06251B), strokeWidth = 2.dp) else Text(text, fontWeight = FontWeight.Black)
    }
}

@Composable
private fun AuthTab(text: String, selected: Boolean, modifier: Modifier, onClick: () -> Unit) {
    Box(modifier.clip(RoundedCornerShape(13.dp)).background(if (selected) Color.White.copy(alpha = .12f) else Color.Transparent).clickable(onClick = onClick).padding(vertical = 11.dp), contentAlignment = Alignment.Center) {
...
private fun AuthMessage(message: String?, error: String?) {
    ...
    if (value != null) {
        Spacer(Modifier.height(10.dp))
        Text(value, color = if (error == null) AuthGreen else Color(0xFFFFB5AD), fontSize = 11.sp, lineHeight = 17.sp)
    }
}
