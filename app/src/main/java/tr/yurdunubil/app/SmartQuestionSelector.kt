...
object SmartQuestionSelector {
    ...
    fun pick(mode: SharedGameMode, prefs: SharedPreferences, seed: Long = System.currentTimeMillis()): List<Question> {
        ...
        val due = candidate.filter { StudyMemory.due(prefs, it.id, now) }
        val dueWrongIds = due.filter { StudyMemory.isWrong(prefs, it.id) }.map { it.id }.toSet()
        val dueWrong = due.filter { it.id in dueWrongIds }
        val dueRest = due.filterNot { it.id in dueWrongIds }
